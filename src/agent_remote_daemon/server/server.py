...
class ProxyServer:
    ...
    def __init__(self, session_name: str, claude_args: list = None,
        ...
        # PTY 相关（mirror 模式下不使用）
        self.master_fd: Optional[int] = None
        self.child_pid: Optional[int] = None
        # Mirror 相关
        self._mirror_pipe_path: Optional[Path] = None
    ...
    async def start(self):
        ...
        t1 = time.time()
        if self.tmux_mirror_target:
            self._start_tmux_mirror()
            logger.info(f"tmux mirror 已启动 ({(time.time()-t1)*1000:.0f}ms)")
        else:
            self._start_pty()
    ...
    def _start_tmux_mirror(self):
        """Mirror 模式：用 tmux pipe-pane 把目标 session 的 PTY 字节流管道到文件

        关键点：
        - tmux pipe-pane -o 输出的是 pane 上看到的原始字节（含 ANSI / blink / cursor），
          不是渲染后的文本。直接喂给 pyte 就能完整还原终端状态。
        - 用一个 FIFO（named pipe）当中转，避免占用磁盘空间
        - 调用方需要确保目标 session 存在且 tmux 可用
        """
        import subprocess as _sp

        target = self.tmux_mirror_target

        # 检查目标 session 存在
        check = _sp.run(["tmux", "has-session", "-t", target],
                        capture_output=True)
        if check.returncode != 0:
            raise RuntimeError(f"tmux session 不存在: {target}")
        ...
        try:
            if pipe_path.exists():
                pipe_path.unlink()
            os.mkfifo(str(pipe_path), 0o600)
        except FileExistsError:
            pass
        self._mirror_pipe_path = pipe_path

        # 启动 tmux pipe-pane（追加模式 -o，shell-quoted cat ... > pipe）
        # 必须在 reader 端打开 FIFO 之前先启动 pipe-pane，否则写端会阻塞
        # 但 mkfifo 创建的 fifo 只有读端打开后写端才能成功——所以我们先以 O_NONBLOCK 打开读端
        self._mirror_fd = os.open(str(pipe_path), os.O_RDONLY | os.O_NONBLOCK)

        # 现在启动 pipe-pane（它会 dup stdout 到 fifo，需要读端已打开）
        shell_cmd = f"cat > {pipe_path}"
        result = _sp.run(["tmux", "pipe-pane", "-t", target, "-o", shell_cmd],
                         capture_output=True, text=True)
        if result.returncode != 0:
            raise RuntimeError(f"tmux pipe-pane 失败: {result.stderr}")

        logger.info(f"tmux mirror 已启动: target={target}, pipe={pipe_path}")

    _COALESCE_MAX = 64 * 1024  # 64KB，防止单次广播过大

    async def _read_tmux_pipe(self):
        """Mirror 模式：读 FIFO 字节流并喂给 OutputWatcher"""
        loop = asyncio.get_event_loop()

        def _read_once() -> Optional[bytes]:
            try:
                return os.read(self._mirror_fd, 4096)
            except BlockingIOError:
                return None
            except OSError:
                return b""

        while self.running and self._mirror_fd is not None:
            try:
                ...
                if data:
                    ...
                    # FIFO 写端关闭，但 tmux pipe-pane 在 session 还活着时会持续写
                    # 这里短暂 sleep，避免空转
                    await asyncio.sleep(0.1)
            except Exception as e:
                if self.running:
                    logger.error(f"读取 mirror pipe 错误: {e}")
                break

        logger.info("Mirror 读取退出")
    ...
    async def _handle_input(self, client_id: str, msg: InputMessage):
        ...
        try:
            data = msg.get_data()
            if self.tmux_mirror_target:
                # Mirror 模式：用 tmux send-keys 发到目标 session
                # -l 表示 literal（不解释 keysym），data 含原始字节包括 ANSI 序列
                import subprocess as _sp
                _sp.run(["tmux", "send-keys", "-t", self.tmux_mirror_target,
                         "-l", data.decode("utf-8", errors="replace")],
                        check=False)
            else:
